    ...
    <section id="navtop">
      ...
      <!-- Barra de navegação acima -->
        ...
        <?php 
            ...
            $descricao_vac = $_POST['descricaovac'];
            $fabricante_vac = $_POST['fabricantevac'];
            $nota_fiscal = $_POST['notafiscal'];
            $lote_vac = $_POST['lotevac'];
            $dataentrada_vac = $_POST['dataentradavac'];
            $validade_vac = $_POST['validadevac'];
            $quantidade_vac = $_POST['quantidadevac'];

            $sql = "INSERT INTO `entrada` (`descri`,`fabricante`,`nota_fiscal`,`lote`,`data_entr`,`validade`,`quant`) VALUES ('$descricao_vac', '$fabricante_vac','$nota_fiscal', '$lote_vac', '$dataentrada_vac', '$validade_vac', '$quantidade_vac')";

        if (mysqli_query($conexao, $sql)) {
        
        ?> <center><br><br><h2 id="cadastroconcluido"> Cadastro feito com sucesso! </h2>
         <a href="cadastro_entradas.php" class="btn btn-primary" style="margin-top: 30px;">Voltar</a></center> <?php
                                            }
