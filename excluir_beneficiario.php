...
<head>
  ...
  <title>Beneficiário excluído com sucesso!</title>
...
<?php
    ...
    $sql = "DELETE FROM `beneficiarios` WHERE id_beneficiarios = $id";

    if (mysqli_query($conexao, $sql)){

    ?><center><br><br><h2 id="cadastroconcluido"> Exclusão feita com sucesso! </h2>
            <a href="listavacinados_editar.php" class="btn btn-primary" style="margin-top: 30px;">Voltar</a></center> <?php
                                            }

            else {

            ?> <center><h2 style="margin-top: 30px;" id="cadastroconcluido"> Erro!! A exclusão não foi feita. </h2>
            <a href="listavacinados_editar.php" class="btn btn-primary" style="margin-top: 30px;">Voltar</a></center> <?php
